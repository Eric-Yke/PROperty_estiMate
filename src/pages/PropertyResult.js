...
import {
  ...
  PiNumberEight,
  PiNumberNine,
} from "react-icons/pi"; // 替换为你实际使用的图标库
...
const PropertyResult = () => {
  ...
  const formattedAddress = property
    ? `${property.house_number} ${titleCase(property.street_name)}, ${titleCase(
        property.property_locality
      )}, NSW ${property.post_code}`
  ...
  const highValue = property
    ? formatPrediction(property.prediction * 1.11)
    : "";
  ...
  const markerIconSvg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24">
            ${
              FaMapMarkerAlt({ color: "red", size: "40px" }).props.children[0]
                .props.children
            }
        </svg>
    `;
  ...
  // 创建SVG标记图标的URL对象
  const svgBlob = new Blob([markerIconSvg], {
  ...
  const handleSearch = (property) => {
    if (property) {
      const { post_code, property_locality, house_number, street_name } =
        property;
      const houseNumberAndStreet = `${house_number}-${street_name}`;
      window.open(
        `/propertyResult/${post_code}/${property_locality}/${houseNumberAndStreet}`,
        "_blank"
      );
    } else {
      console.log(
  ...
  useEffect(() => {
    if (property) {
      const propertyId = property.property_id;
      const apiUrl = `https://www.huanself.top/predict/predict?property_id=${propertyId}`;
  ...
  return (
    <>
      ...
      <div className="card-title">{formattedAddress}</div>
      ...
      <div className="card">
        <div className="filter-buttons">
          <button
            className="button"
            onClick={() =>
              handleButtonClick([2019, 2020, 2021, 2022, 2023, 2024, 2025])
            }
          >
            7 year
          </button>
          <button
            className="button"
            onClick={() =>
              handleButtonClick([2007, 2010, 2013, 2016, 2019, 2023, 2025])
            }
          >
            14 year
          </button>
          <button
            className="button"
            onClick={() =>
              handleButtonClick([2001, 2006, 2011, 2014, 2018, 2023, 2026])
            }
          >
            26 year
          </button>
              ...
              <>
                ...
                <div className="details-item">
                  <span
                    ...
                    style={{ fontSize: "16px", color: "rgb(134, 134, 134)" }}
                  >
                    {property.zoning === "R1"
                      ? "This means less greenery and more noise."
                      : property.zoning === "R2"
                      ? "This means moderate greenery and noise."
                ...
                <div className="details-item">
                  ...
                  >
                    <lighter>
                      {parseFloat(property.area) > 1000
                        ? "超大房子qaq"
                        : parseFloat(property.area) >= 400
                        ? "Comfortable house-size with plenty of room for family to enjoy afternoons."
      ...
      <div className="card-mapView">
        ...
        >
          ...
          <Marker position={center} zIndex={2000} icon={markerIconUrl} />
      ...
      <div className="card-streetView-container">
        {properties.map((property, index) => (
          <div
            key={index}
            className={`card-streetView ${
              index === 0 ? "current-prediction" : "recommended-prediction"
            }`} //给第一个和后两个div分配不同的“类名”来区别当前和推荐的预测目标
            ...
            <div className="card-streetView-text">
              {property ? (
                <>
                  ...
                  <p>Prediction: {formatPrediction(property.prediction)}</p>
                </>
              ) : (
                "???"
              )}
