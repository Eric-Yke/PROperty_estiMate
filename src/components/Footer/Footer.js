...
const Footer = () => {
  ...
  if (location.pathname.startsWith('/propertyResult')) {
    const address = `${houseNumberAndStreet.replace(/-/g, ' ')}, ${propertyLocality}, NSW ${postCode}`;
  ...
  const shareToLinkedIn = () => {
    const linkedInURL = `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(currentUrl)}&title=${encodeURIComponent(text)}&summary=${encodeURIComponent(text)}&source=${encodeURIComponent('YourAppName')}`;
    window.open(linkedInURL, '_blank');
  };

  // 跳转到 Instagram（Instagram 不支持直接从 web 分享）
  const goToInstagram = () => {
    alert("Instagram does not support direct sharing from the web. Please use the Instagram app for sharing.");
  };

  return (
    <div className="footer">
      <div className="footer-section1">
        <Link className="logo" to="/">PRO estiMate</Link>
        <a href="#">Help</a>
        <a href="#">Connect</a>
        <a href="#">About</a>
        <a href="#">Mobile</a>
